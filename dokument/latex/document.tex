...
\subsubsection{Szenario 1: Mobile
Client-Anwendung}\label{szenario-1-mobile-client-anwendung}
...
\subsubsection{Szenario 2: Integration in
Web-Portal}\label{szenario-2-integration-in-web-portal}
...
\subsubsection{Szenario 3: Meta-Suche}\label{szenario-3-meta-suche}

\subsubsection{Szenario 4: Forschungsprojekt Themen- und
Sprachanalyse}\label{szenario-4-forschungsprojekt-themen--und-sprachanalyse}
...
\subsubsection{Der Feed ``Neue Objekte''}\label{der-feed-neue-objekte}
...
\subsubsection{Der Feed ``Geänderte
Objekte''}\label{der-feed-geuxe4nderte-objekte}
...
\subsubsection{Der Feed ``Entfernte
Objekte''}\label{der-feed-entfernte-objekte}
...
\subsection{OParlSystem (System)}\label{oparlsystem-system}
...
\texttt{oparl\_version}}\label{eigenschaft-oparlux5fversion}
...
\subsubsection{Eigenschaft \texttt{bodies}}\label{eigenschaft-bodies}
...
\subsubsection{Eigenschaft \texttt{license}}\label{eigenschaft-license}
...
\subsubsection{Eigenschaft
\texttt{new\_objects}}\label{eigenschaft-newux5fobjects}
...
\subsubsection{Eigenschaft
\texttt{updated\_objects}}\label{eigenschaft-updatedux5fobjects}
...
\subsubsection{Eigenschaft
\texttt{removed\_objects}}\label{eigenschaft-removedux5fobjects}
...
\subsubsection{Eigenschaft \texttt{info}}\label{eigenschaft-info}
...
\subsubsection{Eigenschaft \texttt{vendor}}\label{eigenschaft-vendor}
...
\subsubsection{Eigenschaft \texttt{product}}\label{eigenschaft-product}
...
\hyperdef{}{OParlSystemux5fex1}{\label{OParlSystemux5fex1}}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\{}
...
\subsection{oparl:Body (Körperschaft)}\label{oparlbody-kuxf6rperschaft}
...
\subsubsection{Eigenschaft \texttt{name}}\label{eigenschaft-name-1}
...
\subsubsection{Eigenschaft
\texttt{contact}}\label{eigenschaft-contact-1}
...
\subsection{oparl:Committee (Gremium)}\label{oparlcommittee-gremium}
...
\hyperdef{}{committeeux5fex1}{\label{committeeux5fex1}}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\{}
...
\subsection{oparl:Person (Person)}\label{oparlperson-person}
...
\subsection{oparl:Meeting (Sitzung)}\label{oparlmeeting-sitzung}
...
(Tagesordnungspunkt)}\label{oparlagendaitem-tagesordnungspunkt}
...
\paragraph{Anmerkungen}\label{anmerkungen-2}
...
\subsubsection{Beziehungen}\label{beziehungen-4}
...
\subsection{oparl:Paper (Drucksache)}\label{oparlpaper-drucksache}
...
\subsubsection{Beziehungen}\label{beziehungen-5}
...
\subsection{oparl:Document (Datei)}\label{oparldocument-datei}
...
(\texttt{oparl:Paper}) oder Sitzungen (\texttt{oparl:Meetign}) in
...
\subsection{oparl:Consultation
(Beratung)}\label{oparlconsultation-beratung}

Der Objekttyp \texttt{oparl:Consultation} dient dazu, die Beratung einer
Drucksache (\texttt{oparl:Paper}) in einer Sitzung abzubilden. Dabei ist
es nicht entscheidend, ob diese Beratung in der Vergangenheit
stattgefunden hat oder diese für die Zukunft geplant ist.
...
\subsection{oparl:Location (Ort)}\label{oparllocation-ort}
...
\subsection{oparl:Contact (Kontakt)}\label{oparlcontact-kontakt}
