...
Felix Ebert, Jan Erhardt, Jayan Areekadan, Jens Klessmann, Andreas
...
Objekte grundsätzlich in dieser kompakten Form ausliefern. Die
Umwandlung in die anderen Formen bei Bedarf obliegt dem
Client.\footnote{Die JSON-LD-Spezifikation enthält Überlegungen,
  zwischen Client und Server die gewünschte Form über einen
  \texttt{Accept}-Header mit zusätzlichem \texttt{profile} Parameter
  auszuhandeln. Diese Überlegung ist zur Reduktion der Komplexität auf
  Serverseite nicht in OParl eingefllossen.}
...
\begin{Highlighting}[]
\NormalTok{[}
    \StringTok{"https://oparl.beispielris.de/bodies/0/papers/2"}\NormalTok{,}
    \StringTok{"https://oparl.beispielris.de/bodies/0/papers/5"}\NormalTok{,}
...
Das Prinzip ``Content Negotiation'' wird in RFC2295\footnote{RFC2295:
...
\texttt{httpRange-14} stößt. Wenn man in einem üblichen Web-Browser
diese URL aufruft:

\url{http://dbpedia.org/resource/John_Doe_(musician)}

dann erfolgt eine Weiterleitung auf diese URL:

\url{http://dbpedia.org/page/John_Doe_(musician)}
...
\url{http://dbpedia.org/data/John_Doe_(musician).jsonld}
...
\NormalTok{\{}
    ...
    \DataTypeTok{"fileName"}\NormalTok{: }\StringTok{"57739.pdf"}\NormalTok{,}
...
kann. Typ: ASCII-Zeichenkette. ZWINGEND.
\item[\texttt{name}]
...
sein. Typ: Zeichenkette. EMPFOHLEN.
\item[\texttt{mimeType}]
...
der Veröffentlichung ausgegeben werden. Typ: Datum. ZWINGEND.
\item[\texttt{lastModified}]
...
Änderungszeitpunkt. Typ: Datum. ZWINGEND.
\item[\texttt{size}]
...
\hyperref[dateizugriff]{Dateizugriff}. Typ: URL. ZWINGEND.
\item[\texttt{downloadUrl}]
URL zum Download der Datei. Näheres unter
\hyperref[dateizugriff]{Dateizugriff}. Typ: URL. EMPFOHLEN.
\item[\texttt{paper}]
...
Kategorien vorgegeben. Typ: \texttt{skos:Concept}. OPTIONAL. TODO: Link
auf Erkärungs-Kapitel, damit klar ist, dass hier auf externes Vokabular
verlinkt wird.
\item[\texttt{classification}]
Begriff mit \texttt{skos:prefLabel}. Hat allgemeinere Bedeutung als
\texttt{documentRole}. Typ: \texttt{skos:Concept}. OPTIONAL. TODO: TODO:
Link auf Erkärungs-Kapitel, damit klar ist, dass hier auf externes
